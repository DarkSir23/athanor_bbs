...
class AthanorBBSBoard(HasBoardOps, AthanorOptionScript):
    re_name = re.compile(r"(?i)^([A-Z]|[0-9]|\.|-|')+( ([A-Z]|[0-9]|\.|-|')+)*$")
    lockstring = "reader:all();poster:all();moderator:false();operator:false()"
    examine_type = 'bbs_board'
    examine_caller_type = 'account'
    ...
    def create_bridge(self, category, key, clean_key, order):
        if hasattr(self, 'bbs_board_bridge'):
            return
        BBSBoardBridge.objects.create(db_script=self, db_name=clean_key, db_category=category.bbs_category_bridge,
    ...
    def prefix_order(self):
        bridge = self.bbs_board_bridge
        return f'{bridge.category.db_abbr}{bridge.db_order}'

    @property
    def main_posts(self):
        return self.bbs_board_bridge.posts.filter(parent=None)

    def character_join(self, character):
        self.bbs_board_bridge.ignore_list.remove(character)

    def character_leave(self, character):
        self.bbs_board_bridge.ignore_list.add(character)
